...
export class Node {
  ...
  public numberMachines = 1;

  public parent: Node;
  public childs: Node[] = [];
  public showOptions = false;
  public category = 'unknown'; // crafting, crafting-with-fluid, advanced-crafting, smelting, chemistry
  public machines: any[] = [];
  ...
  public initialize() {
    ...
    const machines = this.dataService.getAssemblingMachinesByCategory(this.category, ingredients.length);
  ...
  public addChild(name: string, recipeRequest: number) {
    const node = new Node(this.dataService, this.plannerService);
    node.parent = this;
    node.name = name
